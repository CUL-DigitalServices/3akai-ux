...
define(['jquery', 'oae.core', 'jquery.fileupload', 'jquery.iframe-transport', 'jquery.jpulse'], function($, oae) {
    ...
    return function(uid) {
        ...
        var initWelcome = function() {
            var hasOpenedWhatVersionBefore = localStorage.getItem('avocet-clicked-what-version');
            ...
            // Check whether the user has clicked the 'What version to upload' button in the past
            if (!hasOpenedWhatVersionBefore) {
                // If not, animate it to make it more prominent
                $('.oa-submitpublication-btn-help i', $rootel).jPulse({
                    color: '#106470',
                    size: 80,
                    speed: 1000,
                    interval: 1000
            ...
            // Flip an up/down arrow icon when a Bootstrap collapsable area expands/collapses + disabled pulsing animation
            $('#oa-submitpublication-welcome-help', $rootel).on('show.bs.collapse hide.bs.collapse', function() {
                var $icon = $('.oa-submitpublication-btn-help i', $rootel).toggleClass('icon-chevron-down icon-chevron-up');
                if (!hasOpenedWhatVersionBefore) {
                    $icon.jPulse('disable');
                    try {
                        // Update local storage. This needs to be within a try catch block
                        // because Safari fails on this line in private browsing mode.
                        localStorage.setItem('avocet-clicked-what-version', true);
                    } catch(e) {}
                }
            });
        };
